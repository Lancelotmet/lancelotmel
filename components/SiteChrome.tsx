...
export function SiteChrome({ children }: { children: ReactNode }) {
  ...
  return <>
    <header className="topbar market-topbar lancelot-market-chrome"><div className="shell topbar-inner"><Link className="brand lancelot-market-brand" href="/"><LancelotCrownMark /><span className="lancelot-market-lockup"><strong>LANCELOT</strong><small>Desde el ser para el saber</small></span></Link><nav className="nav" aria-label="Navegación principal"><Link href="/">Inicio</Link><Link href="/marketplace">Explorar</Link><Link href="/marketplace?category=english">Idiomas</Link><Link href="/marketplace?kind=bundle">Colecciones</Link><Link href="/my-library">Mi biblioteca</Link><Link className="lancelot-market-enter" href="/login">Ingresar <span aria-hidden="true">→</span></Link></nav></div></header>
    {children}
