...
export function SpellingLab() {
  ...
  return <main className="spelling-lab">
    <header className="lab-nav"><Link href="/portal" className="lab-return">← Catálogo</Link><div className="lab-brand"><span>L</span><strong>LANCELOT</strong><small>THE SMALL TRIP · SPELLING</small></div><button onClick={() => setResetOpen(true)} type="button">Reiniciar edición</button></header>
...
function Intro({ name, onName, onBegin }: { name: string; onName: (value: string) => void; onBegin: () => void }) { return <section className="lab-cover"><p className="lab-kicker">Edición interactiva · A2</p><h1>Spelling Lab:<br /><em>Regular Past -ed</em></h1><p>Descubre qué cambia cuando agregamos <b>-ed</b>. No memorices una lista: toma decisiones, compara letras y deja que la siguiente escena responda a lo que necesitas practicar.</p><label>Tu nombre <span>(opcional)</span><input value={name} onChange={(event) => onName(event.target.value)} placeholder="¿Cómo quieres que te llamemos?" autoCapitalize="words" /></label><button className="lab-solid lab-start" onClick={onBegin} type="button">Comenzar <span>→</span></button><small>Una edición del bloque <strong>The Small Trip</strong> · Spelling</small></section>; }

function Diagnostic({ verb, index, answer, onAnswer, onSubmit }: { verb: VerbItem; index: number; answer: string; onAnswer: (value: string) => void; onSubmit: (event: FormEvent) => void }) { return <section className="lab-stage"><p className="lab-kicker">Entrada privada · decisión {index + 1} de {diagnostic.length}</p><h1>Antes de explicarte algo, quiero ver cómo decides.</h1><p className="lab-prompt">{verb.sentence || `${verb.base} → ?`}</p><AnswerForm answer={answer} onAnswer={onAnswer} onSubmit={onSubmit} action="Registrar mi forma" /><p className="lab-note">No hay nota numérica. Solo estamos leyendo tu punto de partida.</p></section>; }
...
function Unit({ rule, verb, stage, errors, answer, feedback, onAnswer, onSubmit, onHypothesis, onNext, onModel }: { rule: RuleId; verb: VerbItem; stage: Stage; errors: number; answer: string; feedback: string; onAnswer: (value: string) => void; onSubmit: (event: FormEvent) => void; onHypothesis: (index: number) => void; onNext: (stage: Stage) => void; onModel: () => void }) {
  ...
  if (stage === "observe") return <section className="lab-stage"><p className="lab-kicker">Observa · {meta.title}</p><h1>No mires una respuesta: <em>mira un patrón.</em></h1><WordObservations items={meta.observe} rule={rule} /><button className="lab-solid" onClick={() => onNext("compare")} type="button">Comparar letras →</button></section>;
  if (stage === "compare") return <section className="lab-stage"><p className="lab-kicker">Compara</p><h1>{meta.discovery}</h1><CompareRule rule={rule} onContinue={() => onNext("discover")} /></section>;
  if (stage === "discover") return <section className="lab-stage"><p className="lab-kicker">Descubre</p><h1>Elige la hipótesis que explica lo que viste.</h1><div className="lab-hypotheses">{meta.hypothesis.map((item, index) => <button key={item} onClick={() => onHypothesis(index)} type="button"><span>{String.fromCharCode(65 + index)}</span>{item}</button>)}</div></section>;
  if (stage === "rule") return <section className="lab-stage lab-rule-reveal"><p className="lab-kicker">Norma rápida</p><h1>{meta.shortRule}</h1><p>La regla apareció después de tu intento, observación y comparación. Ahora puedes usarla en verbos nuevos.</p><button className="lab-solid" onClick={() => onNext("apply")} type="button">Aplicar en otra escena →</button></section>;
  if (stage === "model") { const example = firstVerb(rule, 1); return <section className="lab-stage lab-model"><p className="lab-kicker">Ejemplo paralelo resuelto</p><h1>Observa el cambio, sin prisa.</h1><WordBuild verb={example} /><p>{example.explanation}</p><button className="lab-solid" onClick={onModel} type="button">Probar con un verbo nuevo →</button></section>; }
  const mode = stage === "transfer" ? "Transfiere" : stage === "apply" ? "Aplica" : "Intenta";
  return <section className="lab-stage"><p className="lab-kicker">{mode} · {meta.title}</p><h1>{stage === "transfer" ? (verb.sentence || `Lleva ${verb.base} a una oración.`) : verb.sentence || `${verb.base} → ?`}</h1>{errors > 0 && <Scaffold verb={verb} level={errors} onPick={onAnswer} />}<AnswerForm answer={answer} onAnswer={onAnswer} onSubmit={onSubmit} action={stage === "transfer" ? "Cerrar esta escena" : "Ver mi decisión"} /><p className="lab-note">{stage === "apply" ? "Cada verbo nuevo te permite confirmar la regla sin repetir la misma escena." : "Una sola decisión a la vez."}</p>{feedback && <p className="lab-inline-feedback" aria-live="polite">{feedback}</p>}</section>;
}
